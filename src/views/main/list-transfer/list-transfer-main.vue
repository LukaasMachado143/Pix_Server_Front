<template>
  <div>
    <redirect-add-balance v-if="balanceAccount && balanceAccount <= 100" />
    <card-carousel-transfer
      v-for="(transfers, index) in transfersList"
      :key="index"
      :class="index == transfersList.length - 1 ? '' : 'mb-4'"
      :data="transfers"
    />
    <v-row no-gutters justify="center" class="mt-4 px-4">
